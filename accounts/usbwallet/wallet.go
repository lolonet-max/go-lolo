...
type wallet struct {
	...
	deriveChain     lolo.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain lolo.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
...
// supported for hardware wallets, so this method will alwlolo return an error.
...
// data is not supported for Ledger wallets, so this method will alwlolo return
// an error.
