...
func newClient(ctx *cli.Context) *loloclient.Client {
	client, err := loloclient.Dial(ctx.GlobalString(nodeURLFlag.Name))
	if err != nil {
...
func newContract(client *rpc.Client) (common.Address, *checkpointoracle.CheckpointOracle) {
	...
	contract, err := checkpointoracle.NewCheckpointOracle(addr, loloclient.NewClient(client))
	if err != nil {
