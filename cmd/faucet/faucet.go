...
import (
	...
	"github.com/lolo/go-lolo/loloclient"
...
var (
	...
	statsFlag   = flag.String("lolotats", "", "Ethstats network monitoring auth string")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		if period%60 == 0 {
			...
			if period%24 == 0 {
				period /= 24
				periods[i] = fmt.Sprintf("%d dlolo", period)
			}
		}
		if period == 1 {
...
type faucet struct {
	...
	client *loloclient.Client   // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the lolotats monitoring and reporting service'
	if stats != "" {
		if err := lolotats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := loloclient.NewClient(api)
...
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/lolo/go-lolo/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
