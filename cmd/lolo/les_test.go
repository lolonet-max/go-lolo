...
func (g *lolorpc) killAndWait() {
	g.lolo.Kill()
	g.lolo.WaitExit()
}

func (g *lolorpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.lolo.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *lolorpc) addPeer(peer *lolorpc) {
	g.lolo.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.lolo.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.lolo.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.lolo.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.lolo.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *lolorpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func (g *lolorpc) waitSynced() {
	// Check if it's synced now
	var result interface{}
	g.callRPC(&result, "eth_syncing")
	syncing, ok := result.(bool)
	if ok && !syncing {
		g.lolo.Logf("%v already synced", g.name)
		return
	}

	// Actually wait, subscribe to the event
	ch := make(chan interface{})
	sub, err := g.rpc.Subscribe(context.Background(), "lolo", ch, "syncing")
	if err != nil {
		g.lolo.Fatalf("%v syncing: %v", g.name, err)
	}
	defer sub.Unsubscribe()
	timeout := time.After(4 * time.Second)
	select {
	case ev := <-ch:
		g.lolo.Log("'syncing' event", ev)
		syncing, ok := ev.(bool)
		if ok && !syncing {
			break
		}
		g.lolo.Log("Other 'syncing' event", ev)
	case err := <-sub.Err():
		g.lolo.Fatalf("%v notification: %v", g.name, err)
		break
	case <-timeout:
		g.lolo.Fatalf("%v timeout syncing", g.name)
		break
	}
}

func startGethWithIpc(t *testing.T, name string, args ...string) *lolorpc {
	g := &lolorpc{name: name}
	args = append([]string{"--networkid=42", "--port=0", "--nousb"}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)
	g.lolo = runGeth(t, args...)
	// wait before we can attach to it. TODO: probe for it properly
	time.Sleep(1 * time.Second)
	var err error
	ipcpath := filepath.Join(g.lolo.Datadir, "lolo.ipc")
...
func startLightServer(t *testing.T) *lolorpc {
	datadir := initGeth(t)
...
func startClient(t *testing.T, name string) *lolorpc {
	datadir := initGeth(t)
