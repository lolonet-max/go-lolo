...
// lolo is the official command-line client for Ethereum.
package main
...
const (
	clientIdentifier = "lolo" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the go-lolo command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2020 The go-lolo Authors"
...
// lolo is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func lolo(ctx *cli.Context) error {
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	// Create a client to interact with local lolo node.
	rpcClient, err := stack.Attach()
	if err != nil {
		utils.Fatalf("Failed to attach to self: %v", err)
	}
	ethClient := loloclient.NewClient(rpcClient)
