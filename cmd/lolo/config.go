...
type loloConfig struct {
	Eth      eth.Config
	Shh      whisperDeprecatedConfig
	Node     node.Config
	Ethstats lolotatsConfig
}

func loadConfig(file string, cfg *loloConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "lolo")
	cfg.WSModules = append(cfg.WSModules, "lolo")
	cfg.IPCPath = "lolo.ipc"
	return cfg
}

// makeConfigNode loads lolo configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, loloConfig) {
	// Load defaults.
	cfg := loloConfig{
	...
	if file := ctx.GlobalString(configFileFlag.Name); file != "" {
		...
		if cfg.Shh != (whisperDeprecatedConfig{}) {
			log.Warn("Deprecated whisper config detected. Whisper has been moved to github.com/lolo/whisper")
		}
	}
...
func checkWhisper(ctx *cli.Context) {
	for _, flag := range whisperFlags {
		if ctx.GlobalIsSet(flag.GetName()) {
			log.Warn("deprecated whisper flag detected. Whisper has been moved to github.com/lolo/whisper")
		}
	}
}

// makeFullNode loads lolo configuration and creates the Ethereum backend.
