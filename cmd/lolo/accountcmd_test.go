...
func TestAccountListEmpty(t *testing.T) {
	lolo := runGeth(t, "account", "list")
	lolo.ExpectExit()
}

func TestAccountList(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	lolo := runGeth(t, "account", "list", "--datadir", datadir)
	defer lolo.ExpectExit()
...
func TestAccountNew(t *testing.T) {
	lolo := runGeth(t, "account", "new", "--lightkdf")
	defer lolo.ExpectExit()
	lolo.Expect(`
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	lolo := runGeth(t, "account", "import", keyfile, "-password", passwordFile)
	defer lolo.ExpectExit()
	lolo.Expect(expected)
}
...
func TestWalletImport(t *testing.T) {
	lolo := runGeth(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer lolo.ExpectExit()
	lolo.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := ioutil.ReadDir(filepath.Join(lolo.Datadir, "keystore"))
	if len(files) != 1 {
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(lolo.StderrText(), m) {
...
// https://github.com/lolo/go-lolo/issues/1785
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	lolo.SetTemplateFunc("keypath", func(file string) string {
