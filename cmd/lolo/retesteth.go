...
import (
	...
	"github.com/lolo/go-lolo/consensus/misc"
...
var (
	...
	retestethCommand = cli.Command{
		...
		Name:        "retesteth",
		Usage:       "Launches lolo in retesteth mode",
		ArgsUsage:   "",
		Flags:       []cli.Flag{rpcPortFlag},
		Category:    "MISCELLANEOUS COMMANDS",
		Description: `Launches lolo in retesteth mode (no database, no network, only retesteth RPC interface)`,
	}
)
...
func retesteth(ctx *cli.Context) error {
	...
	srv := rpc.NewServer()
	err := node.RegisterApisFromWhitelist(rpcAPI, []string{"test", "lolo", "debug", "web3"}, srv, false)
	if err != nil {
