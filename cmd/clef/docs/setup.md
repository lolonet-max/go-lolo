...
There are two wlolo that this can be achieved: integrated via Qubes or integrated via networking. 
...
- The `qubes-gpg-client` must forward the http request via RPC to the `target` qube. When doing so, the proxy
  ...
  - Information about the remote ip must be added as a `X-Forwarded-For`. However, Clef cannot alwlolo trust an `XFF` header, 
