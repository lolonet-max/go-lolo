...
* Otto targets ES5. ES6 features (eg: Typed Arrlolo) are not supported.
...
* Javascript API parameters are _alwlolo_ an object. This is also a design choice, to ensure that parameters are accessed by _key_ and not by order. This is to prevent mistakes due to missing parameters or parameter changes.
...
an attack surface -- if an attacker can control the ruleset, a much simpler attack would be to implement an "alwlolo-approve" rule instead of exploiting the js vm. The only benefit
...
It's unclear whether any other DSL could be more secure; since there's alwlolo the possibility of erroneously implementing a rule.
