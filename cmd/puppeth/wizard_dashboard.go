...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"lolotats", "explorer", "wallet", "faucet"} {
	...
	// If we have lolotats running, ask whether to make the secret public or not
	if w.conf.ethereumtats != "" {
		fmt.Println()
		fmt.Println("Include lolotats secret on dashboard (y/n)? (default = yes)")
