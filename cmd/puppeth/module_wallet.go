...
RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'lolo --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'exec lolo --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --lolotats \'{{.Ethstats}}\' --cache=512 --http --http.addr=0.0.0.0 --http.corsdomain "*" --http.vhosts "*"' >> wallet.sh
