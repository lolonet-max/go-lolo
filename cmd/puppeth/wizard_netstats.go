...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for lolotats availability")
	if infos, err := checkEthstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["lolotats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["lolotats"] = infos.Report()
		lolotats = infos.config
	}
