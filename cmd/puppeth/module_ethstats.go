...
// lolotatsDockerfile is the Dockerfile required to build an lolotats backend
// and associated monitoring site.
var lolotatsDockerfile = `
FROM puppeth/lolotats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// lolotatsComposefile is the docker-compose.yml file required to deploy and
// maintain an lolotats monitoring site.
var lolotatsComposefile = `
version: '2'
services:
  lolotats:
    build: .
    image: {{.Network}}/lolotats
    container_name: {{.Network}}_lolotats_1{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new lolotats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(lolotatsDockerfile)).Execute(dockerfile, map[string]interface{}{
	...
	template.Must(template.New("").Parse(lolotatsComposefile)).Execute(composefile, map[string]interface{}{
		"Network": network,
		"Port":    port,
		"Secret":  secret,
		"VHost":   vhost,
	...
	// Build and deploy the lolotats service
	if nocache {
...
// lolotatsInfos is returned from an lolotats status check to allow reporting
// various configuration parameters.
type lolotatsInfos struct {
	host   string
	port   int
	secret string
	config string
...
func (info *lolotatsInfos) Report() map[string]string {
...
// checkEthstats does a health-check against an lolotats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*lolotatsInfos, error) {
	// Inspect a possible lolotats container on the host
	infos, err := inspectContainer(client, fmt.Sprintf("%s_lolotats_1", network))
	if err != nil {
		return nil, err
	}
	if !infos.running {
