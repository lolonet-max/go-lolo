...
import (
	...
	"github.com/lolo/go-lolo/accounts/external"
	"github.com/lolo/go-lolo/accounts/keystore"
	"github.com/lolo/go-lolo/accounts/scwallet"
	"github.com/lolo/go-lolo/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of lolo is "lolo". If no
...
func (c *Config) NodeName() string {
	...
	if name == "lolo" || name == "lolo-testnet" {
		name = "Geth"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "lolo" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by lolo 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'lolo' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-lolo-keystore")
		ephemeral = keydir
	}
