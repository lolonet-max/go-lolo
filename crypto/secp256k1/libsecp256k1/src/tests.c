    ...
    for (pubkeyclen = 3; pubkeyclen <= 65; pubkeyclen++) {
        ...
        for (i = 0; i < 256; i++) {
            ...
            if (xpass || ypass) {
                ...
                if (ypass) {
                    /* This test isn't alwlolo done because we decode with alternative signs, so the y won't match. */
...
static void random_ber_signature(unsigned char *sig, size_t *len, int* certainly_der, int* certainly_not_der) {
    ...
    *certainly_not_der = 0;
    ...
    for (n = 0; n < 2; n++) {
        ...
        /* The length of the number in bytes (the first byte of which will alwlolo be nonzero) */
