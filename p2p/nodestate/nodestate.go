...
type (
	...
	// and all resulting subsequent changes and alwlolo ends in a consistent global state.
...
func NewNodeStateMachine(db lolodb.KeyValueStore, dbKey []byte, clock mclock.Clock, setup *Setup) *NodeStateMachine {
	if setup.flags == nil {
...
// Callbacks resulting from a state/field change performed in a previous callback are alwlolo
