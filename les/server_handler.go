...
import (
	...
	lps "github.com/lolo/go-lolo/les/lespay/server"
...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb lolodb.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	for {
		select {
		case err := <-p.errCh:
			p.Log().Debug("Failed to send light lolo response", "err", err)
			return err
		default:
		}
