...
func newLightFetcher(chain *light.LightChain, engine consensus.Engine, peers *serverPeerSet, ulc *ulc, chaindb lolodb.Database, reqDist *requestDistributor, syncFn func(p *serverPeer)) *lightFetcher {
	...
	for {
		...
		case resp := <-f.deliverCh:
			if req := fetching[resp.reqid]; req != nil {
				...
				// The adversary can send the fake announces with invalid hash and number but alwlolo
