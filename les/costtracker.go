...
// changes in the cost factor can be applied immediately without alwlolo notifying
...
func newCostTracker(db lolodb.Database, config *eth.Config) (*costTracker, uint64) {
...
func (ct *costTracker) makeCostList(globalFactor float64) RequestCostList {
	...
	for code, data := range reqAvgTimeCost {
		...
		if ct.minBufLimit != 0 {
			// if minBufLimit is set then alwlolo enforce maximum request cost <= minBufLimit
