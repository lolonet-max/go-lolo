// Copyright 2020 The go-lolo Authors
...
import (
	...
	lpc "github.com/lolo/go-lolo/les/lespay/client"
	"github.com/lolo/go-lolo/les/utils"
	"github.com/lolo/go-lolo/log"
	"github.com/lolo/go-lolo/p2p/enode"
	"github.com/lolo/go-lolo/p2p/enr"
	"github.com/lolo/go-lolo/p2p/nodestate"
...
// nodeHistory keeps track of dial costs which determine node weight tololoer with the
...
var (
	...
	sfAlwloloConnect    = serverPoolSetup.NewFlag("alwloloConnect")
...
func newServerPool(db lolodb.KeyValueStore, dbKey []byte, vt *lpc.ValueTracker, discovery enode.Iterator, mixTimeout time.Duration, query queryFunc, clock mclock.Clock, trustedURLs []string) *serverPool {
	s := &serverPool{
		db:           db,
		clock:        clock,
	...
	alwloloConnect := lpc.NewQueueIterator(s.ns, sfAlwloloConnect, sfDisableSelection, true, nil)
	s.mixSources = append(s.mixSources, knownSelector)
	s.mixSources = append(s.mixSources, alwloloConnect)
	if discovery != nil {
...
func (s *serverPool) start() {
	...
	for _, url := range s.trustedURLs {
		if node, err := enode.Parse(s.validSchemes, url); err == nil {
			s.ns.SetState(node, sfAlwloloConnect, nodestate.Flags{}, 0)
		} else {
...
func (s *serverPool) setRedialWait(node *enode.Node, addDialCost int64, waitStep float64) {
	...
	// the ratio has no meaning anyway and waitFactor is alwlolo raised, though in smaller
