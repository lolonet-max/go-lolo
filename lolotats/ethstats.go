...
// Package lolotats implements the network stats reporting service.
package lolotats
...
// backend encompasses the bare-minimum functionality needed for lolotats reporting
...
func New(node *node.Node, backend backend, engine consensus.Engine, url string) error {
	...
	node.RegisterLifecycle(lolotats)
	return nil
}
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["lolo"]; info != nil {
...
func (s *Service) reportLatency(conn *connWrapper) error {
	// Send the current time to the lolotats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to lolotats", "latency", latency)
...
// empty arrlolo instead of returning null for them.
...
func (s *Service) reportBlock(conn *connWrapper, block *types.Block) error {
	...
	log.Trace("Sending new block to lolotats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *connWrapper, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to lolotats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *connWrapper) error {
	...
	log.Trace("Sending pending transactions to lolotats", "count", pending)
...
func (s *Service) reportStats(conn *connWrapper) error {
	...
	log.Trace("Sending node details to lolotats")
