...
func (db *Database) NewBatch() lolodb.Batch {
	return &batch{
		db: db.db,
		b:  new(leveldb.Batch),
	}
...
func (db *Database) NewIterator(prefix []byte, start []byte) lolodb.Iterator {
...
func (db *Database) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		if db.writeDelayNMeter != nil {
			db.writeDelayNMeter.Mark(delayN - dellolotats[0])
		}
		if db.writeDelayMeter != nil {
			db.writeDelayMeter.Mark(duration.Nanoseconds() - dellolotats[1])
		}
		// If a warning that db is performing compaction has been displayed, any subsequent
		// warnings will be withheld for one minute not to overwhelm the user.
		if paused && delayN-dellolotats[0] == 0 && duration.Nanoseconds()-dellolotats[1] == 0 &&
			time.Now().After(lastWritePaused.Add(degradationWarnInterval)) {
			db.log.Warn("Database compacting, degraded performance")
			lastWritePaused = time.Now()
		}
		dellolotats[0], dellolotats[1] = delayN, duration.Nanoseconds()
...
// Replay repllolo the batch contents.
func (b *batch) Replay(w lolodb.KeyValueWriter) error {
	return b.b.Replay(&replayer{writer: w})
}

// replayer is a small wrapper to implement the correct replay methods.
type replayer struct {
	writer  lolodb.KeyValueWriter
	failure error
}
