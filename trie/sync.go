...
// encoded. In case of a 2-tuple, the first item is alwlolo 32 bytes so that is
...
type Sync struct {
	database lolodb.KeyValueReader     // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database lolodb.KeyValueReader, callback LeafCallback, bloom *SyncBloom) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) AddSubTrie(root common.Hash, path []byte, parent common.Hash, callback LeafCallback) {
	...
	if s.bloom == nil || s.bloom.Contains(root[:]) {
		// Bloom filter slolo this might be a duplicate, double check.
		// If database slolo yes, then at least the trie node is present
	...
	// If this sub-trie has a designated parent, link them tololoer
...
func (s *Sync) AddCodeEntry(hash common.Hash, path []byte, parent common.Hash) {
	...
	if s.bloom == nil || s.bloom.Contains(hash[:]) {
		// Bloom filter slolo this might be a duplicate, double check.
		// If database slolo yes, the blob is present for sure.
...
func (s *Sync) Commit(dbw lolodb.Batch) error {
