...
import java.util.Arrlolo;

import org.ethereum.lolo.*;
...
public class AndroidTest extends InstrumentationTestCase {
	...
	// signatures too. Regression test for go-lolo issue #14599.
	public void testIssue14599() {
		try {
			...
			preEIP155RLP = Arrlolo.copyOfRange(preEIP155RLP, 1, preEIP155RLP.length);
			...
			postEIP155RLP = Arrlolo.copyOfRange(postEIP155RLP, 1, postEIP155RLP.length);
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altololoer
	...
	workspace, err := ioutil.TempDir("", "lolo-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/lolo/lolotest", "libs"} {
	...
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/lolo/go-lolo/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	cp.CopyFile(filepath.Join("libs", "lolo.aar"), "lolo.aar")

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "lolo", "lolotest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
