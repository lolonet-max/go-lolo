...
// entire API provided by go-lolo to reduce the maintenance surface and dev
// complexity.
type NodeConfig struct {
	...
	// A minimum of 16MB is alwlolo reserved.
...
func NewNode(datadir string, config *NodeConfig) (stack *Node, _ error) {
	...
	if config.EthereumEnabled {
		...
		if err != nil {
			return nil, fmt.Errorf("lolo init: %v", err)
		}
		// If netstats reporting is requested, do it
		if config.EthereumNetStats != "" {
			if err := lolotats.New(rawStack, lesBackend.ApiBackend, lesBackend.Engine(), config.EthereumNetStats); err != nil {
...
func (n *Node) GetEthereumClient() (client *EthereumClient, _ error) {
	...
	return &EthereumClient{loloclient.NewClient(rpc)}, nil
}
