...
// Package lolo contains the simplified mobile APIs to go-lolo.
//
// The scope of this package is *not* to allow writing a custom Ethereum client
// with pieces plucked from go-lolo, rather to allow writing native dapps on
...
// from the go-lolo repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-lolo version.
...
// variant should alwlolo be used as value types crossing over between language
...
// should not be provided to limit the remote code complexity. Arrlolo should be
...
// ret_0 for them, please alwlolo assign names to output variables if tuples.
//
