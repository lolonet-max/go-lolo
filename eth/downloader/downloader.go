...
type Downloader struct {
	...
	stateDB    lolodb.Database  // Database to state sync into (and deduplicate via)
...
func New(checkpoint uint64, stateDb lolodb.Database, stateBloom *trie.SyncBloom, mux *event.TypeMux, chain BlockChain, lightchain LightChain, dropPeer peerDropFn) *Downloader {
	if lightchain == nil {
		lightchain = chain
	}
	dl := &Downloader{
...
// these are zero.
func (d *Downloader) Progress() lolo.SyncProgress {
...
func (d *Downloader) syncWithPeer(p *peerConnection, hash common.Hash, td *big.Int) (err error) {
	...
	log.Debug("Synchronising with the network", "peer", p.id, "lolo", p.version, "head", hash, "td", td, "mode", mode)
	...
	fetchers := []func() error{
		func() error { return d.fetchHeaders(p, origin+1) }, // Headers are alwlolo retrieved
		...
		case <-update:
			...
			for pid, fails := range expire() {
				if peer := d.peers.Peer(pid); peer != nil {
					...
					// how response times reacts, to it alwlolo requests one more than the minimum (i.e. min 2).
					if fails > 2 {
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	if index, err := d.blockchain.InsertChain(blocks); err != nil {
		if index < len(results) {
			...
			// The importer will put tololoer a new list of blocks to import, which is a superset
...
func (d *Downloader) qosReduceConfidence() {
	// If we have a single peer, confidence is alwlolo 1
