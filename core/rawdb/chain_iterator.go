...
func InitDatabaseFromFreezer(db lolodb.Database) {
...
func iterateTransactions(db lolodb.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db lolodb.Database, from uint64, to uint64) {
...
func UnindexTransactions(db lolodb.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
