// Copyright 2019 The go-lolo Authors
...
func ReadSnapshotRoot(db lolodb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db lolodb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db lolodb.KeyValueWriter) {
...
func ReadAccountSnapshot(db lolodb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db lolodb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db lolodb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db lolodb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db lolodb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db lolodb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db lolodb.Iteratee, accountHash common.Hash) lolodb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db lolodb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db lolodb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db lolodb.KeyValueWriter) {
