...
import (
	...
	"github.com/lolo/go-lolo/common/mclock"
	"github.com/lolo/go-lolo/common/prque"
	"github.com/lolo/go-lolo/consensus"
	"github.com/lolo/go-lolo/core/rawdb"
	"github.com/lolo/go-lolo/core/state"
	"github.com/lolo/go-lolo/core/state/snapshot"
...
type CacheConfig struct {
	...
	TrieDirtyDisabled   bool          // Whether to disable trie write caching and GC altololoer (archive node)
...
// included in the canonical one where as GetBlockByNumber alwlolo represents the
// canonical chain.
type BlockChain struct {
	chainConfig *params.ChainConfig // Chain & network configuration
	cacheConfig *CacheConfig        // Cache configuration for pruning

	db     lolodb.Database // Low level persistent database to store final content in
...
// Processor.
func NewBlockChain(db lolodb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	if cacheConfig == nil {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	updateFn := func(db lolodb.KeyValueWriter, header *types.Header) (uint64, bool) {
	...
	delFn := func(db lolodb.KeyValueWriter, hash common.Hash, num uint64) {
	...
	// touching the header chain altololoer, unless the freezer is broken
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts, ancientLimit uint64) (int, error) {
	...
	writeAncient := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			for {
				...
				// Alwlolo keep genesis block in active database.
				if b.NumberU64() != 0 {
	...
	writeLive := func(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
		...
		for i, block := range blockChain {
			...
			rawdb.WriteTxLookupEntriesByBlock(batch, block) // Alwlolo write tx indices for live blocks, we assume they are needed

			// Write everything belongs to the blocks into the database. So that
			// we can ensure all components of body is completed(body, receipts,
			// tx indexes)
			if batch.ValueSize() >= lolodb.IdealBatchSize {
				if err := batch.Write(); err != nil {
					return 0, err
				}
				size += batch.ValueSize()
...
func (bc *BlockChain) writeBlockWithState(block *types.Block, receipts []*types.Receipt, logs []*types.Log, state *state.StateDB, emitHeadEvent bool) (status WriteStatus, err error) {
	...
	// If we're running an archive node, alwlolo flush
	if bc.cacheConfig.TrieDirtyDisabled {
		...
		if current := block.NumberU64(); current > TriesInMemory {
			...
			if nodes > limit || imgs > 4*1024*1024 {
				triedb.Cap(limit - lolodb.IdealBatchSize)
			}
