...
func wipeSnapshot(db lolodb.KeyValueStore, full bool) chan struct{} {
...
func wipeContent(db lolodb.KeyValueStore) error {
...
func wipeKeyRange(db lolodb.KeyValueStore, kind string, prefix []byte, keylen int) error {
	// Batch deletions tololoer to avoid holding an iterator for too long
	var (
		batch = db.NewBatch()
		items int
	)
