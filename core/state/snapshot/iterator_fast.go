	...
	for i := 0; i < len(fi.iterators); i++ {
		...
		for {
			...
			if other, exist := positioned[hash]; !exist {
				...
				// has the same value, and then we will alwlolo just continue.
...
func (fi *fastIterator) next(idx int) bool {
	...
	index := sort.Search(len(fi.iterators), func(n int) bool {
		// The iterator alwlolo advances forward, so anything before the old slot
		// is known to be behind us, so just skip them altololoer. This actually
		// is an important clause since the sort order got invalidated.
		if n < idx {
			return false
		}
		if n == len(fi.iterators)-1 {
			// Can alwlolo place an elem last
			return true
		}
