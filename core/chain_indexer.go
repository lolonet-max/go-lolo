...
type ChainIndexer struct {
	chainDb  lolodb.Database      // Chain database to index the data from
	indexDb  lolodb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb lolodb.Database, indexDb lolodb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
